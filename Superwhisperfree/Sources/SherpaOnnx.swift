...
enum SherpaOnnxError: Error, LocalizedError {
  ...
  var errorDescription: String? {
    switch self {
    case .failedToCreateRecognizer:
      return "Failed to create SherpaOnnx offline recognizer - check model paths and file integrity"
    case .failedToCreateStream:
      return "Failed to create offline stream for decoding"
    case .failedToGetResult:
      return "Failed to get recognition result from stream"
    }
  }
}
...
class SherpaOnnxOfflineRecognizer {
  ...
  /// Failable initializer that throws instead of crashing
  /// - Parameter config: Configuration for the offline recognizer
  /// - Throws: SherpaOnnxError.failedToCreateRecognizer if initialization fails
  init(config: UnsafePointer<SherpaOnnxOfflineRecognizerConfig>) throws {
    guard let ptr = SherpaOnnxCreateOfflineRecognizer(config) else {
      throw SherpaOnnxError.failedToCreateRecognizer
    }
    self.recognizer = ptr
  }
  ...
  /// - Throws: SherpaOnnxError if stream creation or result retrieval fails
  func decode(samples: [Float], sampleRate: Int = 16_000) throws -> SherpaOnnxOfflineRecongitionResult {
    guard let stream = SherpaOnnxCreateOfflineStream(recognizer) else {
      throw SherpaOnnxError.failedToCreateStream
    }
    ...
    guard let resultPtr = SherpaOnnxGetOfflineStreamResult(stream) else {
      throw SherpaOnnxError.failedToGetResult
    }
